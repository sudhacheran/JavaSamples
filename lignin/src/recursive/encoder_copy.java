...
public class encoder_copy {
	
	 //Inputs 
	static String[] bonds = { "B04", "B5", "55", "A04","BB" }; // Bonds possbile
	static int[] bondper = { 45, 15, 10, 10, 10, 10 };  // Ratio of the bonds

	static String[] mono = { "G", "H", "S" }; // Monomer units
	static int[] monoPer = {100, 0, 0 }; // Ratio of the units
	
	static int dp = 5; // No of monomer units
	...
	public static void main(String s[])
	{
		...
		try {
		PrintWriter writer;		
		writer = new PrintWriter("DataSet_copy.txt", "UTF-8");
	...
	{	
		...
		{	 
			 if (!Arrays.asList(m1.getEdgeType()).contains(edgArr.get(j)))
			  {
